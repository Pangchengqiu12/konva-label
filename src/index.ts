...
export default class KonvaCanvas {
  ...
  private labelInfo: Konvalabel.LabelInfo = { label: '' }; //新绘制标签的名称
  private transformer: Konva.Transformer | null = null; //标注框变形
  private onChange: Konvalabel.OnChange | null = null;
  private labelConfig: Konvalabel.LabelConfig = LABEL_CONFIG; //标注框的配置
  ...
  private createLabel({ id, x, y, width, height, labelInfo = this.labelInfo }) {
    const fill = toRgba(
      (this.labelInfo.color || this.labelConfig.color) as string,
      this.labelConfig.fillOpacity
    );
    const stroke = this.labelInfo.color || this.labelConfig.color;
    ...
    const text = new Konva.Text({
      id,
      x,
      y: y - this.labelConfig.textGap!,
      fontSize: this.labelConfig.fontSize! * this.zoomRatio,
      text: labelInfo.label || '未知',
      fill: stroke,
    });
    this.layer.add(text);
    this.layer.add(rect);
  ...
  private updateLable() {
    ...
    this.bboxes.forEach((item, index) => {
      ...
      texts[index]?.y(item.box[1] * this.zoomRatio - this.labelConfig.textGap!);
    });
  }
  /**
   * 鼠标左键按下事件(开始绘制)
  ...
  private mouseup = () => {
    ...
    if (width < 5 || height < 5) {
      ...
      // 为新创建的矩形添加拖动约束
      this.dragBoundFunc(this.currentShape);
  ...
  private updateText = debounce((rect: Konva.Rect) => {
    ...
    find?.setAttrs({
      x: rect.x(),
      y: rect.y() - this.labelConfig.textGap!,
  ...
  private emitChange(type: Konvalabel.ChangeType) {
    ...
    this.bboxes = rects.map((item, index) => {
      const obj: Konvalabel.BboxesItem = { ...item.attrs.data };
      const x = item.x() / this.zoomRatio;
      const y = item.y() / this.zoomRatio;
      const width = item.width() / this.zoomRatio;
      const height = item.height() / this.zoomRatio;
      const box: Konvalabel.BoxType = [x, y, x + width, y + height];
      obj.box = box;
  ...
  //取消绘制标注框
  public cancelDraw() {
    this.labelInfo = { label: '' }; //重置新绘制标签的名称
  ...
  public updateFillOpacity(number: number) {
    ...
    rects.forEach((item) => {
      if (
        ...
        // item.setAttr('opacity', config.value.fillOpcity / 100);
      }
    });
  }
